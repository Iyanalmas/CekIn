...
def show():
	...
	with col_input:
			text = st.text_area(
				...
				height=68,   # ← sebelumnya 160, sekarang diperkecil biar slim
			)

	with col_action:
		submitted = st.button("Prediksi", type="primary", use_container_width=True)
	...
	if submitted:
		...
		result = predict_news(cleaned_text, kernel=kernel or DEFAULT_KERNEL)
		_log_prediction(cleaned_text, result)

		color = confidence_color(result["confidence"])
		level = confidence_level(result["confidence"])

		# ── Divider + 3 kotak hasil (Hasil Prediksi | Valid/Hoax | Confidence)
		...
		# ── Kotak lebar: hasil judul setelah preprocessing
